...
### Runtime / environment
- **Start sim → SLAM → Nav2 → agent within 5 wall-seconds of sim ready.** Starting 30+ sim-seconds late floods every Nav2 node with TF_OLD_DATA at ~56 Hz and drags RTF from ~2.0 to ~0.4. Use `scripts/start_stack.sh` or delegate to the `arst-runner` agent — never start the stack manually.
