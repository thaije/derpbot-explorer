...
- **MPPI `consider_footprint: true` requires an explicit polygon `footprint` on the costmap** — `robot_radius` alone is *not* sufficient. CostCritic throws "Inconsistent configuration in collision checking ... no robot footprint provided" at `on_configure`. Set `footprint: "[[x,y],...]"` on both local + global costmap (chassis 30×20cm + 1cm margin = `[[0.16,0.11],[0.16,-0.11],[-0.16,-0.11],[-0.16,0.11]]`). robot_radius can stay set alongside it.
...
- **Differential inflation (Task 1)** — global 0.55 m / cost_scaling=3.0 (wide soft zone so SLAM corrections don't land the robot in LETHAL cells — eliminates START_OCCUPIED cascade). Local 0.35 m / cost_scaling=5.0 (narrow + moderate decay so MPPI finds low-cost paths through 1 m doors; cost at 0.28 m from wall ≈ 187). Old uniform 0.25 m left only 3 cm soft zone → root cause of post-arrival planning failures. **Do not set local cost_scaling=8.0 without Task 4's `CostCritic.consider_footprint:true`** — point-cost MPPI hits "Failed to make progress" and drops control loop to 4.5 Hz against the steep gradient.
...
- **Custom BT with START_OCCUPIED recovery (Task 1)** — `launch/behavior_trees/navigate_backup_clear_replan.xml`. Contextual planner recovery does `BackUp(0.15 m) → ClearGlobalCostmap` without the `WouldAPlannerRecoveryHelp` gate (default Nav2 BT gates ClearCostmap behind that check, which doesn't match START_OCCUPIED → recovery never fires). NO system-level RoundRobin ClearCostmap loop (that's what previously killed collision_monitor via DDS flood in the stock BT). Bounded retries (`number_of_retries=2`) so failures propagate back to frontier_explorer fast.
- **RotationShimController + PoseProgressChecker pair (Task 2)** — `controller_server.FollowPath` is `nav2_rotation_shim_controller::RotationShimController` wrapping MPPI as `primary_controller`. **Must pair with `nav2_controller::PoseProgressChecker`**: `SimpleProgressChecker` only counts linear displacement, so the shim's in-place rotation is treated as "no progress" → "Failed to make progress" fires after 10 s → cascades into local clears + "Costmap timed out" loops → coverage collapses (verified: 32% in run 1 vs 53% with PoseProgressChecker). Shim engages on 20° heading mismatch (`angular_dist_threshold: 0.35`) — 45° was too loose, leaving slow MPPI rotations on 20–45° cases.
- **`collision_monitor` lifecycle isolation (Task 3)** — `collision_monitor` lives in its own `lifecycle_manager_safety` group (separate from `lifecycle_manager_navigation`) so a planner/controller failure cannot cascade-shutdown the safety path. Both managers use `bond_timeout: 10.0` (the previous 60s value was masking the real issue) and `attempt_respawn_reconnection: True` (Jazzy rename — `attempt_respawn_on_failure` is silently ignored). The lifecycle manager does NOT expose `bond_heartbeat_period` on Jazzy — it's hardcoded in the bond library. `collision_monitor` supports `use_realtime_priority: true` but enabling it requires `/etc/security/limits.conf` grant of `rtprio` (deferred).
- **MPPI footprint-aware critic + tighter sampling (Task 4)** — `CostCritic.consider_footprint: true` (with explicit polygon footprint), `vx_std: 0.15`, `wz_std: 0.3`, `publish_critics_stats: true`. Score 50.6 → 54.9 D, coverage 54.4% → 71.3%, goals 3 → 5+, 1 → 0 collisions on the easy/seed=42/--no-perception baseline. **Local `cost_scaling_factor: 8.0` does not pay back even with footprint-aware MPPI** — controller is stable but first_move regresses ~5–10s per goal (median 2.1s → 11s) and coverage drops 3pp. Stay on csf=5.0.
- **"Start occupied" cascade — fixed in Task 1** — robot arrived at frontier → SLAM mapped new nearby walls → 0.25 m inflation marked robot's cell LETHAL → SmacPlanner refused every subsequent goal. Fixed by wider global inflation (0.55 m, soft zone protects against this) + custom BT recovery. Verified zero START_OCCUPIED events across both Task 1 runs (2026-04-09). Python `_recover_from_occupied_start` safety-net in `frontier_explorer.py` retained for now; remove once BT recovery proven across more runs.
...
config/
  ...
  derpbot_nav2_params.yaml   — robot_radius: 0.22, global inflation=0.55/csf=3.0, local inflation=0.35/csf=5.0, custom BT for START_OCCUPIED recovery
```
